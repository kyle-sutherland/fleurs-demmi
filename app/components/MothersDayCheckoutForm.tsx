...
export function MothersDayCheckoutForm({ arrangements, t }: Props) {
  ...
  const [selectedIds, setSelectedIds] = useState<string[]>(
    firstAvailable ? [firstAvailable.variationId]
    : arrangements[0] ? [arrangements[0].variationId]
    : [],
  );
  const [selectionError, setSelectionError] = useState<string | null>(null);
  const [showCard, setShowCard] = useState(false);

  const selectedItems = arrangements.filter((a) =>
    selectedIds.includes(a.variationId),
  );
  const arrangementPrice = selectedItems.reduce((sum, a) => sum + a.price, 0);
  const total =
    arrangementPrice +
    (fulfillment === "delivery" ? DELIVERY_PRICE : 0) +
    (showCard ? CARD_PRICE : 0);
  ...
  async function handleAddToCart(e: React.FormEvent<HTMLFormElement>) {
    ...
    const pickupLabel =
      fulfillment === "pickup" ? "Pick up — May 9th, Mile End" : "Delivery";
  ...
  return (
    <form onSubmit={handleAddToCart} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        {selectionError && (
          <p className="font-sans text-sm text-red-600">{selectionError}</p>
        )}
            ...
            return (
              <div key={section} className="flex flex-col gap-2">
                ...
                {sectionItems.map((a) => (
                  <label
                    key={a.variationId}
                    className={`flex items-center gap-3 ${a.soldOut ? "opacity-40 cursor-not-allowed" : "cursor-pointer"}`}
                  >
                    <input
                      type="checkbox"
                      value={a.variationId}
      ...
      <div className="border-t-2 border-foreground/10 pt-4 flex flex-col gap-1">
        {selectedItems.map((a) => (
          <div
            key={a.variationId}
            className="flex justify-between font-sans text-sm text-foreground/60"
          >
            <span>{a.name}</span>
