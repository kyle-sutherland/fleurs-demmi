...
export function FuneralsForm({ arrangements, t }: Props) {
  ...
  const selectedArrangements = arrangements.filter(
    (a) => selectedId === a.variationId,
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const getString = (key: string) =>
      (data.get(key) as string | null) ?? undefined;
    ...
    const body = {
      ...
      arrangementNames: selectedArrangements
        .map((a) => a.name)
        .concat(hasCustom ? ["Custom Arrangement"] : []),
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-5">
      ...
      <Field
        ...
        name="funeral_date"
        type="date"
        hint={t.funeralDateHint}
        required
      />
      <Field
        label={t.funeralLocation}
        name="funeral_location"
        type="text"
        hint={t.funeralLocationHint}
      />
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-3">
          {arrangements.map((a) => (
            <label
              key={a.name}
              className={`flex items-center gap-3 font-sans text-sm cursor-pointer ${a.soldOut ? "opacity-40 cursor-not-allowed" : ""}`}
            >
              <input
                type="radio"
                name="arrangement"
          ...
          <label className="flex items-center gap-3 font-sans text-sm cursor-pointer">
            ...
            />
            <span>
              {t.customArrangement} — {t.inquireForPricing}
            </span>
          </label>
        </div>
      </div>
