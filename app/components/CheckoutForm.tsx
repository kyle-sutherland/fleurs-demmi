...
export function CheckoutForm({ applicationId, locationId, sdkUrl, total, subscribeLabel, locale, formT, schedulerT, pickupLocation }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <>
            <div className="flex items-center justify-between border-2 border-foreground/20 px-4 py-3 font-sans text-sm">
              <span>
                ...
                {' (…'}{giftCard.gan.slice(-4)}{')'}{' — '}{formT.giftCardBalance}: ${giftCard.balance.toFixed(2)}
              </span>
              <button
                type="button"
            ...
            <div id="gift-card-container" className="bg-transparent px-1 py-1 min-h-[56px]" />
            {!giftCardReady && (
      ...
      <div className="border-t-2 border-foreground/10 pt-5 flex flex-col gap-1">
        {giftCard && (
          <div className="flex justify-between font-sans text-sm text-foreground/60">
            <span>{formT.giftCardApplied} (…{giftCard.gan.slice(-4)})</span>
      ...
      >
        {submitting ? formT.processing : `${formT.pay} $${displayTotal.toFixed(2)}`}
      </button>
    </form>
  )
}
