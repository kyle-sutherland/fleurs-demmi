...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (err instanceof ZodError) {
      const addressIssue = err.issues.find((i) =>
        i.path.includes("deliveryAddress"),
      );
      if (addressIssue)
        return NextResponse.json(
          { error: addressIssue.message },
          { status: 400 },
        );
    }
  ...
  const surchargeItems = cart.items.filter((i) =>
    i.productId.startsWith("delivery-surcharge:"),
  );
  if (surchargeItems.length > 0) {
    const variationIds = surchargeItems.map((i) =>
      i.productId.slice("delivery-surcharge:".length),
    );
    try {
      const [res, attrKeys] = await Promise.all([
        client.catalog.batchGet({ objectIds: variationIds }),
      ...
      for (const surcharge of surchargeItems) {
        ...
        const attrs =
          variation && "customAttributeValues" in variation
            ? (variation.customAttributeValues as
                | Record<string, { numberValue?: string | null }>
                | undefined)
            : undefined;
        const bouquetsRaw = attrs?.[attrKeys.bouquetsKey]?.numberValue;
        const bouquets = bouquetsRaw != null ? Number(bouquetsRaw) : null;
        const expectedPrice = bouquets != null ? 10 * bouquets : null;
        if (
          expectedPrice === null ||
          Math.abs(surcharge.price * surcharge.quantity - expectedPrice) > 0.01
        ) {
          console.error(
            `Delivery surcharge price mismatch: cartPrice=${surcharge.price * surcharge.quantity} expected=${expectedPrice} varId=${varId} bouquetsKey=${attrKeys.bouquetsKey}`,
          );
          return NextResponse.json(
            { error: "Invalid delivery surcharge." },
            { status: 400 },
          );
        }
      }
  ...
  try {
    ...
    if (needsPickup && body.pickupSegments) {
      try {
        ...
        bookingId = await createPickupBooking(
          body.pickupSegments[0],
          customerNote,
        );
      } catch (err) {
    ...
    const [pickupLocationResult, receiptBufferResult] =
    ...
    const receiptBuffer =
      receiptBufferResult.status === "fulfilled"
        ? receiptBufferResult.value
        : null;
    ...
    const safePickup =
      needsPickup && body.pickupStartAt
        ? escapeHtml(
    ...
    const safeDeliveryAddress =
      hasDelivery && body.deliveryAddress
      ...
      .map((item) => {
        ...
        const opts =
          item.options && Object.keys(item.options).length
            ? ` <span style="color:#888;font-size:12px">(${Object.entries(
                item.options,
              )
    ...
    const ownerHtml = `
      ...
      <table style="font-family:sans-serif;font-size:14px;border-collapse:collapse;width:100%;max-width:600px">
        ...
        ${cart.items
          .filter((i) => i.options?.pickup)
          .map(
            (i) =>
              `<tr><td style="padding:6px 12px;border-bottom:1px solid #eee;font-weight:600;color:#555">${escapeHtml(i.name)}</td><td style="padding:6px 12px;border-bottom:1px solid #eee">${escapeHtml(i.options!.pickup)}</td></tr>`,
          )
          .join("")}
      </table>
    ...
    const customerHtml = email
      ...
      <div style="font-family:sans-serif;max-width:600px;color:#1a1a1a">
        ...
        ${receiptUrl ? `<p style="margin-top:24px"><a href="${escapeHtml(receiptUrl)}" style="display:inline-block;padding:10px 20px;background:#1a1a1a;color:#fff;text-decoration:none;font-size:13px;font-weight:600">View receipt</a></p>` : ""}
