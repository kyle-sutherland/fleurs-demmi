...
const bodySchema = z.object({
  ...
  funeral_location:  textSchema.nullish(),
  fulfillment:       z.union([z.literal('pickup'), z.literal('delivery')]).optional(),
  arrangementName:   textSchema.nullish(),
  style_notes:       textSchema.nullish(),
  card_name:         textSchema.nullish(),
  card_message:      textSchema.nullish(),
...
export async function POST(request: Request) {
  ...
  try {
    await Promise.all([
      ...
      upsertSquareCustomer({ name, email, phone, source: 'funerals-inquiry', subscribed: 'unknown', isOrder: false }),
    ])
  } catch (err) {
